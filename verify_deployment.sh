...
size_of_local_elf="$(wc -c target/deploy/plutonium.so | cut -d' ' -f1)"

size_diff="$(( size_of_chain_elf - size_of_local_elf ))"

dd if=/dev/zero of=target/deploy/plutonium.so bs=1 count=$size_diff \
	oflag=append conv=notrunc

chain_csum="$(sha256sum chain_bin.so | cut -d' ' -f1)"
local_csum="$(sha256sum target/deploy/plutonium.so | cut -d' ' -f1)"
...
$local_csum  target/deploy/plutonium.so
*****************************************************************
EOM
	exit 1
fi
